...
import { LocalStorageModuleConfig } from "../LocalStorageModuleConfig";

export class LocalStorageManager extends BaseObject {
    ...
    protected createLocalStorage(): Storage {
        // Workaround for weird cases when local storage is not available
    ...
    protected getWindowStorageObject(): any {
        let rawData: string = this.localStorage.getItem(this.storageId);
        if (!rawData) {
            rawData = "{}";
        }
    ...
    protected writeSaves(): void {
        ...
        this.localStorage.setItem(this.storageId, rawData);
    }
    ...
    // protected prepareLocalStorage(): void {
